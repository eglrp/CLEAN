		...
		for (int i = 0; i < mapmatchPathSet->size(); ++i) {
			...
			*fout_cprs << Head_t <<" "<< Head_d<< endl;
			...
			while (j < size-1){
				...
				double start_sum = tempPairs->at(j)->t + tempPairs->at(j)->d - Head - travel_sofar;
				int A = 1;
				int xp = 0;
				while (true) {
					double e2xp = pow(2,xp + 1);
					if (e2xp > start_sum) break;
					int t_A = (int)(start_sum / e2xp);
					// if (abs(t_A * e2xp - start_sum) < Unit_D_T){
					// 	A = t_A;
					// 	xp++;
					// 	continue;
					if (abs((t_A + 1) * e2xp - start_sum) < Unit_D_T){
						A = t_A + 1;
						xp++;
						continue;	
					}
				...
				int delta = A * pow(2, xp + 1);
				// cout << "P " << j << " "<< tempPairs->at(j)->t<<" "<< tempPairs->at(j)->d<< endl;
